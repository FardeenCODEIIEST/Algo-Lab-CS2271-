...
df= pd.read_csv('C:/Users/joyab/Documents/Programming/College/4th Sem/Algorithm/Assignment2/Assignment2_Part2/Shannon_Fano/Result_Record.csv')
...
avg_compression_ratio = round(df["Compression Ratio"].mean(), 6)
...
plt.title("Variation of Entropy and Avg. Shannon-Fano Code Len with Original File Size")

# Plot the lines for entropy and average code length
ax1.plot(df["OG_File(bytes)"], df["Entropy"], color="red", label="Entropy")
ax1.plot(df["OG_File(bytes)"], df["Avg Code Len"], color="blue", label="Avg. Shannon-Fano Code Len",)
